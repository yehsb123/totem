...
| 26 | AUTO-26 | ✅ | 저장 뒤 갱신 실패를 "저장 실패"로 표시 → 입력 창이 남아 다시 누르면 일정·리뷰가 두 번 생김 (일정관리 5개 동작, 리뷰 추가·가져오기·삭제) (재점검 7차) |
| 27 | AUTO-27 | ✅ | 빈 상태 문구: 새 조직 투어관리 "조건에 맞는 투어가 없습니다"(필터 없음), 대시보드가 고객에게 "npm run seed 를 실행해주세요", 일정표 코스 없음이 오류 화면·돌아갈 길 없음 (재점검 7차) |
| 28 | AUTO-28 | ✅ | 입력 한계: 길이·숫자·형식 오류 문구가 영어("String must contain at most 100 character(s)") — 화면 폼과 API 400 모두. 아이디 찾기 이름·전화 상한 없음 (재점검 8차) |
| 29 | AUTO-29 | ✅ | 메인 SEO: robots.txt·sitemap.xml 없음(404), /features 가 임시 이동(307)이라 같은 내용이 두 주소, 코스메이커·일정관리 기능 소개의 제목 단계 건너뜀(h1→h3) (재점검 9차) |
| 30 | AUTO-30 | ✅ | 키보드: 콘솔 공용 모달·메인 로그인 모달에서 Tab 이 뒤 화면으로 빠지고, 닫아도 포커스가 연 버튼으로 돌아오지 않음. 콘솔 모달은 autoFocus 가 없으면 첫 포커스도 없고, 겹친 모달은 Esc 한 번에 모두 닫힘 (재점검 10차) |
...
| 38 | AUTO-38 | ✅ | 결정 대기 항목이 WORKLOG 곳곳에 흩어져 있음 → docs/DECISIONS.md 한 곳에 (급함·정책·외부 서비스·화면 내용, 선택지·권장안·결정 후 할 일) (재점검 17차) |
| 39 | AUTO-39 | ✅ | 데이터 정합: 투어관리에서 투어 날짜·이름·담당자를 바꿔도 연결된 일정(코스메이커로 투어 등록 시 자동 생성)은 옛 값 그대로 → 달력과 투어 목록이 다른 날짜를 보여 줌 (재점검 18차) |
| 40 | AUTO-40 | ✅ | 투어 일정표(PDF)가 **코스 날짜**로 찍힘 — 투어 날짜를 옮기거나 한 코스로 여러 날짜 투어를 만들면 고객에게 틀린 날짜가 나감. 담당·인원은 항상 코스의 첫 투어 (재점검 19차) |
| 41 | AUTO-41 | ✅ | 대시보드: "전달 대비 방문자 증감률"이 내국인만으로 계산(옆 카드 "월 방문자 수"는 내·외국인 합) — 2025-06 -9.6% 표시(실제 -7.5%). 총 관광 소비액이 카드(저장 총액)·탭(항목 합)·차트에서 서로 다름. 중복 항목이 막대 두 개로 (재점검 20차) |
| 42 | AUTO-42 | ✅ | 코스메이커 기간 변경: 일차 내용을 **날짜로** 맞춰, 코스를 다른 달로 옮기면 담은 장소가 전부 사라짐. 시작일을 앞당기면 31일 초과도 그대로(저장 때에야 오류), 줄여서 장소가 지워져도 안내 없음. 시간대 칸 수 상한 없음 (재점검 21차) |
| 43 | AUTO-43 | ✅ | 일정관리 검색이 보이는 달 안에서만 → 다른 달 일정을 검색하면 빈 달력(없는 일정처럼 보임), 결과가 어디 있는지 알 길 없음 (재점검 22차) |
| 44 | AUTO-44 | ✅ | 투어관리: 마지막 페이지의 마지막 투어를 지우면 없는 페이지에 남아 "조건에 맞는 투어가 없습니다"(앞 페이지엔 투어가 있음) (재점검 23차) |
| 45 | AUTO-45 | ✅ | 세션: 메인 사이트가 "로그인됨"을 저장된 토큰 유무로만 판단 → 탈퇴·다른 기기 비밀번호 변경으로 세션이 폐기돼도 헤더가 계속 로그인 상태("콘솔로 이동"). 콘솔은 탈퇴 후 자기 토큰을 지우지 않음 (재점검 24차) |
| 46 | AUTO-46 | ✅ | 문구·입력: 없는 대상 오류가 "계정을(를)·투어을(를)" 처럼 조사 두 개("을(를)"), 설정 누락은 "이(가)". 가입 중 "이미 가입된 이메일 → 로그인"으로 넘어가면 채워진 이메일 칸에 포커스(비밀번호 칸이 맞음) (재점검 25차) |
| 47 | AUTO-47 | ✅ | 리뷰 CSV: 크기 한도를 본문을 전부 받은 뒤 **글자 수**로 검사 — content-length 없는 응답은 끝없이 메모리에 받고, 한글은 한도의 약 3배까지 통과. 결과 화면이 사유 목록이 앞 100건뿐인 걸 알리지 않음, 큰 숫자 구분 쉼표 없음 (재점검 26차) |
| 48 | AUTO-48 | ✅ | 메인: 모바일 메뉴가 Esc 로 안 닫힘, 메뉴 이름 "가격안내" ↔ 페이지·콘솔 "요금제", 가격표의 "무료로 시작하기"가 무엇으로 시작하는지 안내 없음, FAQ·가입 화면 "모든 기능 무료 체험"(Pro 기능은 출시 예정) (재점검 27차) |
