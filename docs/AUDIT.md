...
## 24. 재점검 18차 — 화면 사이 데이터 정합 (2026-09-25)

한 데이터를 여러 화면이 보여 줄 때 값이 같은지: 비정규화 값(투어 리뷰 통계 `$inc`, 장소 인기도, 코스 투어 수), 소프트 삭제 연쇄, 부분 수정 검증.

| # | 영역 | 발견 | 처리 |
|---|---|---|---|
| I1 | 정합 | 투어 날짜·이름·담당자 수정이 연결된 일정에 반영되지 않음 → **투어관리와 일정관리 달력이 다른 날짜** | ✅ 수정 시 연결 일정 맞춤(날짜는 항상, 이름·담당자는 일정에서 따로 고치지 않았을 때만) + (organizationId, tourId) 인덱스. 테스트(수정 전 코드로 실패 확인) (AUTO-39) |

문제없음: 리뷰 통계는 추가·가져오기(일괄)·삭제 모두 같은 증감, 장소 인기도는 코스 생성 때만(재저장으로 부풀지 않음), 좌석 검증은 저장된 값과 합쳐서, 투어 삭제 시 일정은 연결만 해제.

## 25. 재점검 19차 — 코스 ↔ 투어 ↔ 일정표 (2026-09-25)

| # | 영역 | 발견 | 처리 |
|---|---|---|---|
| J1 | 일정표 | 일정표(요금제의 "투어 일정 PDF")가 코스에 저장된 날짜로 일차 날짜를 찍음. 투어 날짜를 옮기거나(AUTO-39 로 가능) 한 코스로 여러 출발 투어를 만들면 **고객에게 나가는 PDF 날짜가 틀림**. 담당·인원도 항상 코스의 첫 투어 | ✅ 투어관리의 "일정표"는 그 투어(`?tourId=`)로 열고, 투어 출발일부터 일차 날짜·요일·기간·제목·담당·인원. 투어 기간 ≠ 코스 일수면 경고(인쇄에는 안 나옴). 코스에 투어가 하나면 그 투어, 여럿이면 코스 날짜. E2E 추가 (AUTO-40) |

## 26. 재점검 20차 — 대시보드 숫자 ↔ 원본 시드 (2026-09-25)

2025-06 을 원본 시드(`db/seed/data/tourism-jeju.ts`)로 직접 계산해 화면과 대조, 12개월 전체의 합계 일관성 검사.

| # | 영역 | 발견 | 처리 |
|---|---|---|---|
| H1 | 계산 | "전달 대비 방문자 증감률"을 **내국인만**으로 계산 — 바로 옆 "월 방문자 수"는 내·외국인 합. 2025-06 화면 -9.6%, 같은 기준이면 -7.5% | ✅ 서버가 총 방문자 기준으로 계산한 값 사용 (AUTO-41) |
| H2 | 계산 | 총 관광 소비액이 세 곳에서 다름: 종합 카드(저장 총액 8,960만) · 국내/국외 탭(항목 합 8,289만+2,810만) · 추이 차트(항목 합) | ✅ **항목 합으로 통일**(서버 종합 현황판 포함) — 탭의 비율과 같은 총액. 화면 확인: 카드 1.1억 = 두 탭 합 (AUTO-41) |
| H3 | 표시 | 같은 항목이 두 번 있는 달(2025-02 카지노, 2025-06 식음료) → 막대·조각 두 개 | ✅ 항목별로 합쳐 표시 (AUTO-41) |
| H4 | 원본 | 시드 자체가 안 맞음: 국내 저장 총액 ≠ 항목 합(최대 35%), 성별·연령 비율 합 ≈114%, 국가 비율 합 최대 114% | ⛔ 원본 검수 필요 → DECISIONS CNT-001 (값을 추측해 고치지 않음) |

고정 장치: `dashboard.test.ts` — 종합 현황판 방문자·증감률·소비액·상위 국가를 원본 시드로 직접 계산한 값과 비교.

## 27. 재점검 21차 — 코스메이커 규칙 경계 (2026-09-25)

서버 규칙(`validateCourseRules`: 기간·31일·일차 날짜 정렬·시간대 범위·칸 중복·숙소 전용 칸·최소 1곳)과 화면 사전 검증(`placementError`·달력 min/max) 대조 — 규칙 자체는 일치. 문제는 기간을 바꿀 때의 화면 동작.

| # | 영역 | 발견 | 처리 |
|---|---|---|---|
| K1 | 데이터 손실 | 기간이 바뀌면 일차 내용을 **날짜로** 유지 → 코스를 다른 날짜로 옮기면(10/01~03 → 11/01~03) 날짜가 하나도 안 맞아 **담은 장소가 전부 사라짐** | ✅ 일차 순서로 유지, 시작일을 바꾸면 일수 그대로 코스를 옮김 (AUTO-42) |
| K2 | 검증 | 시작일을 앞당기면 31일을 넘어도 그대로(종료일 max 는 종료일 칸에만) → 저장 때에야 오류 | ✅ 기간 변경 시 31일로 맞춤 (AUTO-42) |
| K3 | 안내 | 기간을 줄여 장소가 담긴 일차가 지워져도 묻지 않음 | ✅ 확인창(취소하면 그대로) (AUTO-42) |
| K4 | 검증 | 시간대 칸 수 상한 없음 — 한 요청에 수만 칸 저장 가능 | ✅ 최대 48칸(30분 단위 하루) (AUTO-42) |

E2E: 데모 코스를 다른 달로 옮겨도 장소 유지, 줄이면 확인창·취소 시 그대로 — 수정 전 코드로 실패 확인. 전체 실행에서 다른 테스트가 만든 코스를 먼저 집던 선택자도 이름으로 고정.

## 28. 재점검 22차 — 일정관리 달력 경계 (2026-09-25)

여러 달에 걸친 일정(범위 조회 `startDate ≤ to AND endDate ≥ from`·칸별 표시), 6주 달력 범위(앞뒤 달 날짜 포함), 라벨 삭제 시 옮기기(409 → reassignTo) — 정상.

| # | 영역 | 발견 | 처리 |
|---|---|---|---|
| L1 | 검색 | 검색이 **보이는 달 안에서만** — 다른 달의 일정(이름·담당자·장소)을 찾으면 빈 달력만 보여 없는 일정처럼 보임 | ✅ 검색어가 있으면 오른쪽에 **전체 기간 결과 목록**(건수·기간·담당), 누르면 그 달로 이동·상세 열기. 저장·삭제하면 목록도 새로. E2E (AUTO-43) |

## 29. 재점검 23차 — 투어관리 목록 필터·페이지 (2026-09-25)

필터(검색·날짜·타입·상태)를 바꾸면 1페이지로, 전체보기로 초기화, 페이지 정보는 필터 기준 — 정상. 서버 조합 필터·정규식 특수문자·없는 페이지 동작을 테스트로 고정(`tour-list.test.ts`).

| # | 영역 | 발견 | 처리 |
|---|---|---|---|
| M1 | 페이지 | 마지막 페이지의 마지막 투어를 지우면 없는 페이지에 머물러 빈 목록 문구 | ✅ 빈 페이지면 있는 마지막 페이지로. E2E(21개 → 2페이지 1개 삭제 → 1페이지) — 수정 전 코드로 실패 확인 (AUTO-44) |

## 30. 재점검 24차 — 설정 > 계정 관리 (2026-09-25)

프로필 수정(이메일 변경 불가·전화 형식), 비밀번호 변경(현재 비밀번호 확인·같은 비밀번호 거부·전 세션 폐기 → 다시 로그인), 카카오 전용 계정(비밀번호 칸 대신 안내·탈퇴 시 비밀번호 없이), 탈퇴(확인 문구·소유자 + 다른 멤버 있으면 409 안내) — 정상.

| # | 영역 | 발견 | 처리 |
|---|---|---|---|
| A1 | 세션 | 메인 사이트는 저장된 토큰이 **있기만 하면** 로그인 상태로 표시 → 탈퇴·다른 기기 비밀번호 변경으로 세션이 폐기돼도 헤더가 "콘솔로 이동·로그아웃" | ✅ 첫 화면에서 저장된 세션을 한 번 확인, 서버가 거절할 때만 비움(네트워크 오류는 유지). E2E(수정 전 코드로 실패 확인) (AUTO-45) |
| A2 | 세션 | 콘솔이 탈퇴 후 자기 토큰을 지우지 않고 이동 | ✅ 탈퇴 직후 토큰 삭제 (AUTO-45) |

## 31. 재점검 25차 — 로그인·회원가입 오류 안내 (2026-09-25)

브라우저로 확인: 틀린 비밀번호·없는 계정은 **같은 문구**(계정 존재 추측 방지), 이메일 유지, 형식 오류는 보내기 전에, 서버 꺼짐 → "서버에 연결할 수 없습니다"(버튼 다시 사용 가능), 요청 과다(429) 문구, 가입 시 이미 가입된 이메일 → 안내 + 로그인으로 이동 시 이메일 이어 받음 — 정상.

| # | 영역 | 발견 | 처리 |
|---|---|---|---|
| E1 | 문구 | 없는 대상 404 문구가 "일치하는 계정을(를)·투어을(를)" — 조사를 두 개 붙임(12종 전부), 설정 누락 503 은 "이(가)" | ✅ 받침에 맞는 조사(`josa`, 괄호 설명은 건너뜀·한글이 아니면 두 형태). 테스트 (AUTO-46) |
| E2 | 입력 | 가입 → "로그인" 으로 넘어가면 이미 채워진 이메일 칸에 포커스 | ✅ 첫 **빈** 칸(비밀번호)에 포커스 (AUTO-46) |

## 32. 재점검 26차 — 리뷰 CSV 가져오기 (2026-09-25)

허용되지 않은 주소(실제 서버 문구 "허용: docs.google.com, googleusercontent.com"), https 만, 리다이렉트 매 단계 검사, HTML 응답 거부, 헤더에 총점 없음·데이터 행 없음 문구, 행 오류는 행 번호와 함께 — 정상.

| # | 영역 | 발견 | 처리 |
|---|---|---|---|
| R1 | 보안·자원 | 크기 한도(2MB)를 본문을 **전부 받은 뒤** 글자 수로 검사 → content-length 없는(청크) 응답은 끝없이 메모리에 쌓이고, 한글(3바이트/글자)은 약 6MB 까지 통과 | ✅ 스트림으로 읽으며 **바이트**를 세다 넘는 순간 중단. 테스트 3건(100MB 흉내는 30조각 안에서 중단·한글 초과 거부·이모지 포함 조각 경계 정상) — 수정 전 코드로 2건 실패 확인 (AUTO-47) |
| R2 | 화면 | 사유 목록은 서버가 앞 100건만 주는데 화면에 안내 없음 · 3200 처럼 쉼표 없는 큰 숫자 | ✅ "건너뛴 3,000건 중 앞 100건의 사유만" 안내, 숫자 쉼표 (AUTO-47) |

## 33. 재점검 27차 — 메인 모바일 메뉴·요금제 (2026-09-25)

가격표 "무료로 시작하기": 로그인 안 됨 → 가입 창, 로그인됨 → 콘솔 — 정상. 모바일 메뉴: aria-expanded·aria-controls, 이동하면 닫힘, 375px 가로 넘침 없음 — 정상.

| # | 영역 | 발견 | 처리 |
|---|---|---|---|
| W1 | 키보드 | 모바일 메뉴가 Esc 로 닫히지 않음(기능 드롭다운은 닫힘) | ✅ Esc 로 닫힘 + E2E (AUTO-48) |
| W2 | 문구 | 메뉴 "가격안내" ↔ 페이지 제목·콘솔 안내 "요금제" | ✅ "요금제"로 통일 (AUTO-48) |
| W3 | 문구 | 가격표에 유료 요금제만 있고 "무료로 시작하기"가 무엇인지 설명 없음(실제: 무료 체험·기간 제한 없음·결제 없음) · FAQ·가입 화면 "모든 기능 무료 체험"(Pro 기능은 출시 예정) | ✅ "가입하면 무료 체험으로 바로 시작, 유료 전환은 결제 연동 후 안내", "지금 제공되는 기능(Basic)". 체험 기간·조건은 DECISIONS POL-006 (AUTO-48) |
